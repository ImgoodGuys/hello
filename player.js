...
function initializePlayer(client) {
    ...
    client.riffy.on("trackStart", async (player, track) => {
        ...
        const requester = requesters.get(trackUri) || "Unknown";
        ...
        try {
            ...
            .setFooter({ text: "Remake by Huy_RRT", iconURL: musicIcons.heartIcon })
            .setTimestamp()
            .setDescription(  
    ...
    client.riffy.on("queueEnd", async (player) => {
        ...
        try {
            // Safety check for autoplayCollection
            let autoplaySetting = null;
            if (autoplayCollection && typeof autoplayCollection.findOne === 'function') {
                try {
                    autoplaySetting = await autoplayCollection.findOne({ guildId });
                } catch (dbError) {
                    console.error("Database error:", dbError);
                }
            }
    
            if (autoplaySetting && autoplaySetting.autoplay) {
                try {
                    ...
                    console.error("Error in autoplay:", autoplayError);
                    await cleanupTrackMessages(client, player);
                    player.destroy();
                    await channel.send("❌ **Autoplay error. Disconnecting...**");
                }
            } else {
                await cleanupTrackMessages(client, player);
...
async function cleanupTrackMessages(client, player) {
    ...
    for (const messageInfo of messages) {
        try {
            const channel = client.channels.cache.get(messageInfo.channelId) || null;
            if (channel) {
...
async function getLyrics(trackName, artistName, duration) {
    try {
        ...
        console.error("❌ Lyrics fetch error:", error.response && error.response.data && error.response.data.message ? error.response.data.message : error.message);
        return null;
    }
}
